...
var mousemove = function(d) {
    Tooltip
      .html(`District: ${d.District} <br> Total number of 1st newborns: ${d.Number_1} <br> Gender Ratio: ${d.Ratio_1}`)
...
svg.selectAll("circle")
  ...
  .enter()
  .append("circle")
    .attr("cx", function (d) { return xScale(d.Number_1)} )
    .attr("cy", function (d) { return yScale(d.Ratio_1)} )
    .attr("r", 5)
    ...
    if (yScale(d.Ratio_1) < yScale(fit_curve(d.Number_1)))
     {return colorScale(d.Ratio_1-fit_curve(d.Number_1));} 
...
var mousemove2 = function(d) {
    Tooltip
      .html(`District: ${d.properties.name} <br> Total number of 1st newborns: ${d.properties.Number_1}<br>Gender ratio: ${d.properties.Ratio_1}`)
      .style("left", (d3.event.pageX+10) + "px")
      .style("top", (d3.event.pageY) + "px")
  }
...
svg2.selectAll("path")
      ...
      .style("stroke-width", 0.5)
      .style("fill", function(d) {
    if (d.properties.Ratio_1 > fit_curve(d.properties.Number_1))
     {return colorScale(d.properties.Ratio_1-fit_curve(d.properties.Number_1));} 
    // elif d.properties.Ratio_3=null, fill with gray
    else if (d.properties.Ratio_1==null)
